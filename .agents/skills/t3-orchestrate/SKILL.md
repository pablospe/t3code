...
| `spawn --project <p> --title <t> [--base main] [--mode auto] [--model i:m] [--no-worktree] [--plan] <prompt>` | Create a sibling thread in a fresh worktree and send its first prompt; `--plan` runs the provider's native plan mode so the child proposes a plan for approval instead of implementing. On servers with HTTP bootstrap parity this is one server-side request (worktree based on origin/<base> when a remote exists, project setup script runs, rollback on failure); older servers fall back to a client-side sequence that skips the setup script. Prompt text containing `--words` can be protected behind a literal `--` |
...
| `approve <threadId>`                                                                                          | Approve a plan-mode child's proposed plan: flips the thread to default mode and starts the implementation turn carrying the plan (marks it implemented server-side). To re-plan with feedback instead, send `prompt <threadId> <feedback>` - a plan-mode thread re-plans                                                                                                                                                                                                                                                     |
