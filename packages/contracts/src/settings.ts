...
export const ServerSettings = Schema.Struct({
  ...
  // Mirrors Claude CLI sessions running outside T3 Code into read-only threads.
  // Off by default: it polls the `claude` binary while enabled.
  enableAttachedSessions: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(false))),
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  enableAttachedSessions: Schema.optionalKey(Schema.Boolean),
  continueThreadsAfterServerUpdate: Schema.optionalKey(Schema.Boolean),
  enableAgentBrowserAccess: Schema.optionalKey(Schema.Boolean),
